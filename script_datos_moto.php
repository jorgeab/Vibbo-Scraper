<?php
	...
	function script_datos($link){
		...
		if($html != ""){		
			...
			list($id_anuncio, $tipo_vehiculo, $titulo_anuncio, $fecha_creacion, $hora_creacion, $fecha_publicacion, $hora_publicacion, $marca, $tipo_moto, $tipo_combustible, $anyo, $precio, $km) = generar_datos_anuncio($array);
			$provincia = get_provincia($link);
			
			//echo $id_anuncio. " " .$tipo_vehiculo. " " .$titulo_anuncio. " " .$fecha_creacion. " " .$hora_creacion. " " .$fecha_publicacion. " " .$hora_publicacion. " " .$marca. " " .$tipo_moto. " "  .$tipo_combustible. " " .$anyo. " " .$precio. " " .$km. "<br>". $link. "<br>";
			//exit;
			...
			if ($id != "" && $nombre != "" && $telefono != "" && $tipo_usuario != "" && $pais != "" && 
				...
				$tipo_moto != "" && $tipo_combustible != "" && $anyo != "" && $precio != "" && $km != "" && $link != "" && $provincia != ""){
					
					guardar_datos($id, $nombre, $telefono, $tipo_usuario, $pais, $provincia, $id_anuncio, $tipo_vehiculo, $titulo_anuncio, $fecha_creacion, $hora_creacion, $fecha_publicacion, $hora_publicacion, $marca, $tipo_moto, $tipo_combustible, $anyo, $precio, $km, $link);
			}
		}
	}


	function guardar_datos($id, $nombre, $telefono, $tipo_usuario, $pais, $provincia, $id_anuncio, $tipo_vehiculo, $titulo_anuncio, $fecha_creacion, $hora_creacion, $fecha_publicacion, $hora_publicacion, $marca, $tipo_moto, $tipo_combustible, $anyo, $precio, $km, $link){
		
		$datos = $id. ";" .$nombre. ";" .$telefono. ";" .$tipo_usuario. ";" .$pais. ";" .$provincia. ";" .$id_anuncio. ";" .$tipo_vehiculo. ";" .$titulo_anuncio. ";" .$fecha_creacion. ";" .$hora_creacion. ";" .$fecha_publicacion. ";" .$hora_publicacion. ";" .$marca. ";" .$tipo_moto. ";" .$tipo_combustible. ";" .$anyo. ";" .$precio. ";" .$km. ";" .$link. PHP_EOL; //PHP_EOL -> Nueva linea
	...
	function generar_datos_anuncio($array){
		...
		for($c=1; $c<count($array); $c++){
			...
			/****TIPO_MOTO****/
			if(strlen(strstr($array[$c] ,"Tipo:"))){
				$tipo_moto = strip_tags($array[$c+1]);
				//print strip_tags($array[$c+1]);
				//echo "<br>";
			}

			/****TIPO_COMBUSTIBLE****/
			if(strlen(strstr($array[$c] ,"Combustible:"))){
				$tipo_combustible = strip_tags($array[$c+1]);
				//print strip_tags($array[$c+1]);
				//echo "<br>";
			}
		...
		if ($marca == ""){
			$tit_temp = explode(" ", $titulo_anuncio); 
			$marca = $tit_temp[0];
			if($marca == "GAS"){$marca = "GAS GAS";}
		}
		
		if ($km == ""){$km = "200000";}
		
		return array($id_anuncio, $tipo_vehiculo, $titulo_anuncio, $fecha_creacion, $hora_creacion, $fecha_publicacion, $hora_publicacion, $marca, $tipo_moto, $tipo_combustible, $anyo, $precio, $km);
